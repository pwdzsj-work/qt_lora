...
NQ1～NQ4 另外使用标准功能码 `0x02`（Read Discrete Inputs）独立轮询：

```text
请求：01 02 00 00 00 04 79 C9
响应数据：bit0=NQ1、bit1=NQ2、bit2=NQ3、bit3=NQ4
```

Qt 校验地址、功能码和 CRC 后拆分四个状态，通过 `QmlModelShowData`
命令 `6` 上报设备详情页。位值 `1` 显示为绿色“高”，位值 `0` 显示为
灰色“低”。每个终端依次轮询模拟量数据、继电器状态和 NQ 输入状态，
避免多个 LoRa 终端同时发送。
